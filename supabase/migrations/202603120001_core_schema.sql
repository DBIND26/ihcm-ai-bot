...
BEGIN
    IF NOT EXISTS (
        ...
        CREATE DOMAIN public.us_state_code AS CHAR(2)
            CHECK (VALUE ~ '^[A-Z]{2}$');
    END IF;
END $$;
...
CREATE TABLE public.facilities (
    ...
    state_code              public.us_state_code NOT NULL,
...
CREATE INDEX idx_facilities_state ON public.facilities(state_code);
CREATE INDEX idx_facilities_status ON public.facilities(operational_status);

CREATE TABLE public.resident_episodes (
    episode_id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    resident_internal_id    TEXT NOT NULL,
...
CREATE UNIQUE INDEX uq_episode_source
    ON public.resident_episodes(source_system, source_record_id)
    WHERE source_system IS NOT NULL AND source_record_id IS NOT NULL;

CREATE INDEX idx_episodes_facility_status
    ON public.resident_episodes(facility_id, episode_status);
CREATE INDEX idx_episodes_admission_date
    ON public.resident_episodes(admission_date);
...
CREATE TABLE public.mds_assessments (
    ...
    CONSTRAINT fk_mds_episode_facility
        ...
        REFERENCES public.resident_episodes(episode_id, facility_id),
...
CREATE UNIQUE INDEX uq_mds_source
    ON public.mds_assessments(source_system, source_record_id)
    WHERE source_system IS NOT NULL AND source_record_id IS NOT NULL;

CREATE UNIQUE INDEX uq_mds_active_business_key
    ON public.mds_assessments(facility_id, episode_id, assessment_type, due_date, payment_model)
    WHERE status NOT IN ('modified', 'inactivated');

CREATE INDEX idx_mds_facility_status_due
    ON public.mds_assessments(facility_id, status, due_date);
CREATE INDEX idx_mds_episode
    ON public.mds_assessments(episode_id);

CREATE TABLE public.assessment_reimbursement_classifications (
    assessment_classification_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    assessment_id                UUID NOT NULL REFERENCES public.mds_assessments(assessment_id) ON DELETE CASCADE,
...
CREATE INDEX idx_assessment_classifications_assessment
    ON public.assessment_reimbursement_classifications(assessment_id);
CREATE INDEX idx_assessment_classifications_model
    ON public.assessment_reimbursement_classifications(payment_model);

CREATE TABLE public.staffing_daily (
...
CREATE UNIQUE INDEX uq_staffing_source
    ON public.staffing_daily(source_system, source_record_id)
    WHERE source_system IS NOT NULL AND source_record_id IS NOT NULL;

CREATE INDEX idx_staffing_facility_date
    ON public.staffing_daily(facility_id, staffing_date DESC);
CREATE INDEX idx_staffing_issues
    ON public.staffing_daily(staffing_date DESC, below_target_flag);
...
CREATE INDEX idx_alerts_open
    ON public.ai_alerts(status, severity, alert_date DESC);
CREATE INDEX idx_alerts_facility_category
    ON public.ai_alerts(facility_id, alert_category);

CREATE TABLE public.facility_risk_scores (
...
CREATE INDEX idx_risk_facility_date
    ON public.facility_risk_scores(facility_id, score_date DESC, created_at DESC);
...
CREATE TABLE public.incident_events (
    ...
    CONSTRAINT chk_incident_due_date
        CHECK (investigation_due_date IS NULL OR investigation_due_date >= incident_date::DATE),
    CONSTRAINT chk_incident_complete_timestamp
        CHECK (investigation_status <> 'complete' OR investigation_completed_at IS NOT NULL)
);

CREATE UNIQUE INDEX uq_incident_source
    ON public.incident_events(source_system, source_record_id)
    WHERE source_system IS NOT NULL AND source_record_id IS NOT NULL;

CREATE INDEX idx_incidents_facility_date
    ON public.incident_events(facility_id, incident_date DESC);
CREATE INDEX idx_incidents_type
    ON public.incident_events(incident_type);
CREATE INDEX idx_incidents_investigation_status
    ON public.incident_events(investigation_status);

CREATE TABLE public.reimbursement_events (
    reimbursement_event_id      UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    episode_id                  UUID,
    assessment_id               UUID REFERENCES public.mds_assessments(assessment_id),
    facility_id                 UUID NOT NULL REFERENCES public.facilities(facility_id),
...
CREATE UNIQUE INDEX uq_reimbursement_source
    ON public.reimbursement_events(source_system, source_record_id)
    WHERE source_system IS NOT NULL AND source_record_id IS NOT NULL;

CREATE INDEX idx_reimbursement_facility_date
    ON public.reimbursement_events(facility_id, event_date DESC);
CREATE INDEX idx_reimbursement_status
    ON public.reimbursement_events(status);
CREATE INDEX idx_reimbursement_type
    ON public.reimbursement_events(event_type);
...
CREATE TABLE public.daily_brief_facilities (
...
BEFORE UPDATE ON public.resident_episodes
FOR EACH ROW
EXECUTE FUNCTION public.set_updated_at();

CREATE TRIGGER trg_mds_assessments_updated_at
BEFORE UPDATE ON public.mds_assessments
FOR EACH ROW
EXECUTE FUNCTION public.set_updated_at();
...
BEFORE UPDATE ON public.reimbursement_events
FOR EACH ROW
EXECUTE FUNCTION public.set_updated_at();

CREATE OR REPLACE VIEW public.v_facility_risk_current
...
CREATE OR REPLACE VIEW public.v_alerts_open
WITH (security_invoker = true) AS
SELECT
    a.alert_id,
    a.facility_id,
...
CREATE OR REPLACE VIEW public.v_mds_control_tower
WITH (security_invoker = true) AS
SELECT
    m.assessment_id,
    m.facility_id,
...
JOIN public.facilities f ON f.facility_id = m.facility_id
WHERE m.status NOT IN ('submitted', 'inactivated')
ORDER BY m.due_date ASC, f.facility_name, m.assessment_id;

CREATE OR REPLACE VIEW public.v_staffing_issues
WITH (security_invoker = true) AS
SELECT
    s.staffing_id,
    s.facility_id,
...
CREATE OR REPLACE VIEW public.v_revenue_leakage
WITH (security_invoker = true) AS
SELECT
    r.facility_id,
    f.facility_name,
...
FROM public.reimbursement_events r
...
CREATE OR REPLACE VIEW public.v_incident_trending
WITH (security_invoker = true) AS
SELECT
    i.facility_id,
    f.facility_name,
...
FROM public.incident_events i
JOIN public.facilities f ON f.facility_id = i.facility_id
